...
class Climaoro(hass.Hass):
    ...
    Args attesi in apps.yaml:
        ha_url:   base URL di HA (es. http://192.168.1.132:80)
        ha_token: token long-lived di HA
    ...
    def initialize(self):
        self.ha_url = self.args.get("ha_url", "http://192.168.1.132:80").rstrip("/")
    ...
    def _fetch_config(self):
        ...
        req = urllib.request.Request(url, headers={"Authorization": f"Bearer {self.ha_token}"})
